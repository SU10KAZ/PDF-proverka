...
def change_existence(raw, transition, comparison, sufficiency, valid_witnesses, issues):
    ...
    value_ids = {side: set(getattr(transition, side).state_value_evidence_ids) for side in ('old', 'new')}
    ...
    if isinstance(semantic, dict):
        ...
        grounded = bool(refs) and refs <= valid_witnesses and bool(refs & value_ids['old']) and bool(refs & value_ids['new'])
    ...
    return dict(status=value, source=source, explanation=reason, counter_evidence=counter_receipt,
        ...
        state_value_evidence_ids={side: sorted(value_ids[side]) for side in ('old', 'new')},
        subject_identity_evidence_ids={side: list(getattr(transition, side).subject_identity_evidence_ids)
                                      for side in ('old', 'new')})
