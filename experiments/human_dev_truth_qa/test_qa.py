...
class QATests(unittest.TestCase):
    ...
    def test_blind_completion_disagreements_resolution_and_final_freeze(self):
        ...
        self.store.save(request(self.packet,ids[1],"YES","review"))
        self.assertFalse((self.directory/FINAL_NAME).exists())
        req=request(self.packet,ids[2],"YES","review")
    ...
    def test_all_matching_freezes_after_last_human_answer_and_publication_recovers(self):
        self.complete()
        self.assertEqual(self.store.state()["disagreements"],[])
        self.assertEqual(self.store.state()["phase"],"final")
        self.assertTrue((self.directory/FINAL_NAME).exists())
        final=json.loads(self.store.export(final=True))
        self.assertEqual(final["provenance"][-1]["annotator"],"system:human-dev-qa")
        self.assertEqual(final["human_decision_trigger_sha256"],final["provenance"][-2]["event_sha256"])
