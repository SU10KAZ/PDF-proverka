...
PREVIOUS = ROOT / 'controlled_inference_f1_f4_f2_v1'
OUT = ROOT / 'controlled_inference_f1_f4_f2_v2'
...
def previous_hashes():
    return {str(p.relative_to(PREVIOUS)): sha(p) for p in sorted(PREVIOUS.rglob('*')) if p.is_file()}
