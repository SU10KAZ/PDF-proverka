...
from experiments.project_change_contracts_272.comparability import applicable_dimensions, known, materiality
from .negative import bounded_contract, certify
from .sufficiency import evaluate
from experiments.project_change_contracts_272.witnesses import raster_locator_errors

VERSION = 'F2_MODEL_NORMALIZATION/2'
...
def conflict(raw, subject, role, claim_id):
    ...
    targets = raw.get('affected_claims', [])
    ...
    return dict(status=status, relevance=relevance, affected_claims=targets,
...
def compare(transition):
    ...
    # Missing engineering phase on both sides is not missing version direction.
    # An explicitly specified phase on one side still requires comparison.
    if not known(transition.old.stage_phase) and not known(transition.new.stage_phase):
    ...
    for name in sorted(dimensions):
        ...
        conditions.append(dict(dimension=name, old=a, new=b, comparison=status, applicability='APPLICABLE'))
...
def normalize(raw, packet, profile):
    ...
    for side in ('old', 'new'):
        ...
        if not ids or not set(ids) <= delivered.keys():
            issues.append(side + '_UNDELIVERED_STATE_EVIDENCE')
        if not set(ids) & valid_witnesses:
        ...
        if isinstance(identity, dict) and all(known(identity.get(k)) for k in ('functional_owner', 'system', 'subsystem', 'scope')):
            ...
            if set(identity.get('evidence_ids', [])) <= set(ids) and identity.get('evidence_ids'):
                ...
                record(side + '.engineering_subject', source['engineering_subject'], canonical,
                    'Explicit functional identity bound to this side state witnesses', ids)
            else:
    ...
    if effective == 'ACCEPT' and (issues or not sufficiency['sufficient'] or significance['status'] != 'MATERIAL'):
        effective = 'REVIEW'
    return dict(schema=VERSION, status='REVIEW' if issues else 'NORMALIZED', issues=issues,
