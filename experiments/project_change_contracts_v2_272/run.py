...
from .sufficiency import VERSION as SUFFICIENCY_VERSION
...
def freeze():
    ...
    code_files = [p for name in ('project_change_contracts_v2_272', 'project_change_contracts_272',
    ...
    dirty = __import__('subprocess').check_output(['git', '-C', str(CODE), 'status', '--porcelain', '--',
        'experiments/project_change_contracts_v2_272'], text=True)
    if dirty:
    ...
    manifest = dict(schema='CONTROLLED_V2_MANIFEST/2', code_commit=git_commit(), cases=[dict(pair_index=p, case_id=c) for p, c in CASES],
        ...
        prior_run=ref(PREVIOUS / 'CONTROLLED_INFERENCE_MANIFEST.json'))
    write(OUT / 'CONTROLLED_V2_MANIFEST.json', manifest)
    frozen = dict(schema='ARCHITECTURE_FIX_FREEZE/2', code_commit=git_commit(), runtime=runtime,
        manifest=ref(OUT / 'CONTROLLED_V2_MANIFEST.json'), prompt=ref(OUT / 'PROMPT.txt'),
    ...
    for path in artifact_files + [OUT / 'ARCHITECTURE_FIX_FREEZE.json', OUT / 'CONTROLLED_V2_MANIFEST.json']:
        path.chmod(0o444)
    verify()
