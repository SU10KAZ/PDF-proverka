...
from .numeric import NumericConflictGuard
...
def repair(raw, packet, context, facts=()):
    ...
    if app['supporting_witness']['applicability'] == 'NOT_APPLICABLE' and not n['source_conflict']['blocking'] and not numeric['blocking']:
    ...
    final = 'REVIEW' if app['issues'] or numeric['blocking'] else admission['effective_verdict']
    n.update(schema='PAIR_A_POST_INFERENCE_REPAIR/2', status='REVIEW' if issues else 'NORMALIZED', issues=issues,
        ...
        admission_order=['NORMALIZATION_V1', 'BINDING_V1', 'APPLICABILITY_V2', 'NUMERIC_CONFLICT_GUARD', 'F2', 'V4', 'FINAL_ADMISSION'])
    n['transformations'].append(dict(path='applicability_v2', before=None, after=app,
