...
from .modality import graphic_delivery, requirements
...
def evaluate(raw, packet, states, v1, source_conflict):
    ...
    return dict(claim_type=kind, contract='NEGATIVE' if negative else 'POSITIVE', NEGATIVE_SCOPE=scope,
        ...
        supporting_witness=dict(role='SUPPORTING_WITNESS', applicability=witness, category=category,
            ...
            witness_required_for=reasons,
            witness_required_modality=('GRAPHIC' if any(x in reasons for x in
                ['VISUAL_ENDPOINT_NOT_DELIVERED', 'AMBIGUOUS_SUBJECT_LOCATION_OR_ENDPOINT']) and spatial
                else 'TEXT_ALLOWED' if required else 'NOT_REQUIRED'),
        ...
        evidence_role_requirements=requirements(raw, packet, states, spatial, arithmetic),
