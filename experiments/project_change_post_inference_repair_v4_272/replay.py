...
from .applicability import NEGATIVE_CONTRACT
from .modality import RULES
...
V3 = ROOT / 'fresh_dev_pair_a_post_inference_repair_v3'
OUT = ROOT / 'fresh_dev_pair_a_post_inference_repair_v4'
...
def verify_inputs(metadata_only=False):
    files = read(OUT / 'INPUT_FREEZE.json')['files']
    changed = [] if metadata_only else [p for p,h in files.items() if sha(p) != h]
    if changed: raise ValueError('Frozen input drift: ' + str(changed))
    metadata = read(OUT / 'INPUT_FREEZE.json')['immutable_tree_metadata']
    current = {str(p): [p.stat().st_size, p.stat().st_mtime_ns] for d in (LIVE, V1, V2, V3) for p in d.rglob('*') if p.is_file()}
    if metadata != current: raise ValueError('Immutable prior artifact metadata changed')
    return dict(immutable_prior_artifacts=len(current), files=len(files), unchanged=True, metadata_only=metadata_only,
                raw_responses=sum('/raw_responses/' in p for p in files if p.startswith(str(LIVE))))


def initialize():
    if OUT.exists():
        raise FileExistsError('V4 output already exists; replay is single-use')
    # Read byte hashes, never source truth content, before rule/result freeze.
    files = dict(read(V3/'INPUT_FREEZE.json')['files'])
    for path, digest in files.items():
        assert sha(path) == digest, path
    for root in (LIVE, V1, V2, V3):
        for p in root.rglob('*'):
            if p.is_file():
                files[str(p)] = sha(p)
    save('INPUT_FREEZE.json', dict(files=files, source_truth_opened=False,
        immutable_tree_metadata={str(p): [p.stat().st_size, p.stat().st_mtime_ns]
            for root in (LIVE,V1,V2,V3) for p in root.rglob('*') if p.is_file()}))
...
def offline_guard():
    ...
    def guard(event, args):
        ...
        if event == 'open' and isinstance(args[0],(str,bytes)):
            ...
            if path.is_relative_to(BASE) and 'pair_8' in path.parts:
                raise PermissionError('Pair B is not authorized')
            if path.is_relative_to(LIVE) or path.is_relative_to(V1) or path.is_relative_to(V2) or path.is_relative_to(V3) or path.is_relative_to(BASE):
...
def role_audits(rows, previous):
    requirements, changes, negatives = [], [], []
    for r in rows:
        if not r['raw']:
            continue
        n = r['normalized']; a = n['claim_applicability']
        old = previous[r['package_id']]['normalized']['claim_applicability']
        role_rows = a['evidence_role_requirements']
        requirements.extend(dict(candidate_id=r['package_id'], **q) for q in role_rows)
        if 'GRAPHIC_REQUIRED_NOT_DELIVERED' in previous[r['package_id']]['normalized']['issues']:
            for q in role_rows:
                inherited = q['evidence_id'] in old['graphic']['sides'].get(q['side'],{}).get('required_ids',[])
                changes.append(dict(candidate_id=r['package_id'],claim=r['raw']['project_change_summary'],
                    evidence=q['evidence_id'],evidence_role=q['evidence_role'],
                    V3_required_modality='GRAPHIC' if inherited else 'ROLE_NATIVE',
                    V4_required_modality=q['required_modality'],actual_modality=q['actual_modality'],
                    final_verdict=r['status'],explanation=q['requirement_reason'],requirement=q))
        if r['raw']['verdict']=='NOT_CHANGE':
            negatives.append(dict(candidate_id=r['package_id'],claim=r['raw']['project_change_summary'],
                V3_final_verdict=previous[r['package_id']]['status'], final_verdict=r['status'],
                scope=a['NEGATIVE_SCOPE'],scope_unchanged=a['NEGATIVE_SCOPE']==old['NEGATIVE_SCOPE'],
                primary=a['primary_evidence'],graphic=a['graphic'],witness=a['supporting_witness'],
                evidence_roles=role_rows,issues=n['issues']))
    save('EVIDENCE_ROLE_REQUIREMENTS_V4.json',dict(schema='EVIDENCE_ROLE_REQUIREMENTS/4',rows=requirements))
    save('MODALITY_BEFORE_AFTER.json',dict(rows=changes,source_truth_used=False))
    save('NEGATIVE_CONTRACT_AUDIT.json',dict(rows=negatives,
        status='PASS' if all(r['scope_unchanged'] and (r['final_verdict']!='NOT_CHANGE' or
            (r['primary']['complete'] and not r['issues'] and
             all(d['complete'] for d in r['graphic']['sides'].values()))) for r in negatives) else 'FAIL'))
    # Regression-only lookup, after general rules and tests. Never an admission input.
    regression=[r for r in rows if r['package_id']=='c_6eaf92b25145b86cd05df50c']
    assert len(regression)==1
    r=regression[0]; a=r['normalized']['claim_applicability']
    primary=[q for q in a['evidence_role_requirements'] if q['evidence_role']=='PRIMARY_STATE_EVIDENCE']
    text=[q for q in a['evidence_role_requirements'] if q['actual_modality']=='TEXT']
    save('STAIRCASE_REGRESSION_CHECK.json',dict(candidate_id=r['package_id'],
        regression_only=True,rule_has_case_specific_inputs=False,claim=r['raw']['project_change_summary'],
        primary_graphic_evidence=primary,supporting_text_evidence=text,final_verdict=r['status'],
        V3_final_verdict=previous[r['package_id']]['status'],
        status='PASS' if primary and text and r['status']=='NOT_CHANGE' and
            all(q['required_modality']=='TEXT_ALLOWED' for q in text) and
            all(d['complete'] for d in a['graphic']['sides'].values()) else 'FAIL'))


TEST_MODULES = [
    'experiments.project_change_post_inference_repair_v4_272.test_repair',
...
def workbook(rows, groups, before_after, negatives):
    ...
    for r in negatives:
        neg.append([r['candidate_id'],r['raw_negative_claim'],json.dumps(r['scope'],ensure_ascii=False),r['V3_final_verdict'],r['final_verdict'],
    ...
    path=OUT/'PAIR_A_REPAIRED_V4_SYSTEM_OUTPUT.xlsx'
...
def main():
    ...
    for module in ['project_change_post_inference_repair_v2_272','project_change_post_inference_repair_v3_272','project_change_f2_binding_v4_272','project_change_contracts_272','project_change_contracts_v2_272','project_change_semantic_codex_v2_272']:
    ...
    save('MODALITY_APPLICABILITY_RULES.json',dict(rules=RULES,source_truth_used=False))
    ...
    save('PAIR_A_REPAIRED_V4_RESULTS.json',rows)
    previous_accept={k for k,v in previous.items() if v['status']=='ACCEPT'}
    structural=dict(
        eleven_v3_accepts_preserved=len(previous_accept)==11 and previous_accept <= {r['package_id'] for r in rows if r['status']=='ACCEPT'},
        ...
        no_new_binding=all(json.loads(json.dumps(r['normalized'][side+'_state']['evidence_bindings']))==previous[r['package_id']]['normalized'][side+'_state']['evidence_bindings'] for r in rows if r['raw'] for side in ('old','new')), 
        ...
        accepted_dispositive=all(any(q['evidence_role']=='PRIMARY_STATE_EVIDENCE' and q['side']==side
            and q['requirement_status']=='SATISFIED' for q in r['normalized']['claim_applicability']['evidence_role_requirements'])
            for r in rows if r['status'] in {'ACCEPT','NOT_CHANGE'} for side in ('old','new')),
        accepted_graphics_both_sides=all(all(d['complete'] and d['required_ids'] for d in
            r['normalized']['claim_applicability']['graphic']['sides'].values())
            for r in rows if r['status'] in {'ACCEPT','NOT_CHANGE'} and
            r['normalized']['claim_applicability']['graphic']['requirement']=='GRAPHIC_REQUIRED'),
    ...
    assert len(four)==2
    save('RAW_ACCEPT_2_BEFORE_AFTER.json',four)
    negatives=[]
    for r in rows:
        ...
        negatives.append(dict(candidate_id=r['package_id'],raw_negative_claim=r['raw']['project_change_summary'],
            ...
            V3_blocker=reasons(previous[r['package_id']]['normalized']), V3_final_verdict=previous[r['package_id']]['status'],
    ...
    save('POST_INFERENCE_REGRESSION_AUDIT.json',dict(status='PASS' if all(structural.values()) else 'FAIL',checks=structural,
        ...
        truth_evaluation='PENDING_POST_REPLAY',raw_hashes=verify_inputs(metadata_only=True)))
    role_audits(rows, previous)
