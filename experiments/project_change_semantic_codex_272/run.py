...
async def execute(name, smoke=False, parallel=2):
    ...
    identity = dict(name=name, partition='DEV', smoke=smoke, config=config, parallelism=parallel,
        ...
        code=code_identity(), proposal_prompt_sha256=digest(PROPOSE),
    ...
    async def one(row):
        async with sem:
            if stopped.is_set() or provider.stopped:
                return
            key = row['packet_id']
            ...
            try:
                if target.exists() and done.exists():
                    ...
                    result = await infer_packet(packets[key], call, PROPOSE)
...
if __name__ == '__main__':
    ...
    result = asyncio.run(execute(args.name, args.smoke, args.parallel))
