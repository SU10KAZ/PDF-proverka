...
CAPABILITY_BLOCK_DETECTOR_SOL = "block_detector_sol"
CAPABILITY_BLOCK_DETECTOR_ASTRA = "block_detector_astra"
...
KNOWN_CAPABILITIES: tuple[str, ...] = (
    ...
    CAPABILITY_BLOCK_DETECTOR_SOL,
    CAPABILITY_BLOCK_DETECTOR_ASTRA,
...
class ProviderRequirementPayload(BaseModel):
    ...
    #:
    #: Граница поднята на 11I вместе с `CENTER_MAX_INFERENCES`: прежние 64
    #: описывали одноногий worker-участок, а нескхлопнутый ансамбль этапа 01
    #: даёт четыре обращения на графический блок. См. комментарий к константе.
    max_inferences: int = Field(default=0, ge=0, le=1024)
...
class AuditPipelineParams(BaseModel):
    ...
    #: Версия контракта обязательности frozen routing plan.
    #:
    #: Поле отделяет настоящий legacy job (поля ещё не существовало) от нового
    #: задания, у которого план потерян или повреждён. Без явного маркера оба
    #: случая выглядят как ``routing_plan=None`` и центральный хвост вынужден
    #: либо ломать старые задания, либо fail-open читать live config.
    routing_plan_contract_version: Optional[Literal[1]] = None
    #: ЗАМОРОЖЕННЫЙ план маршрутизации (этап 11I). Канонический источник
    #: маршрута для задания: какие действия, каким провайдером, какой
    #: способностью, в каком порядке и сколько раз.
    #:
    #: Почему словарь, а не вложенная Pydantic-модель. План — самостоятельная
    #: доменная сущность со своим номером схемы, своей канонической
    #: сериализацией и своим хэшем; продублировать её вторым набором классов
    #: значило бы получить два описания одного контракта, которые разъедутся
    #: на первой правке. Строгость здесь обеспечивает не форма поля, а
    #: валидатор ниже: он разбирает план настоящим разбором и отвергает любой,
    #: который не проходит доменную проверку.
    #:
    #: `provider_requirement` при этом СОХРАНЯЕТСЯ: он остаётся тем, что
    #: понимает уже установленный воркер (одна пара «провайдер + способность»,
    #: белый список этапов, потолок вызовов). План его не отменяет, а уточняет.
    routing_plan: Optional[dict[str, Any]] = None

    @field_validator("routing_plan")
    @classmethod
    def _check_routing_plan(cls, value: Optional[dict[str, Any]]) -> Optional[dict[str, Any]]:
        """План обязан разбираться и проходить доменную проверку ЗДЕСЬ.

        Проверка живёт в схеме нагрузки, а не в вызывающем коде, по той же
        причине, что и запрет точной модели: иначе «в задание попадает только
        валидный план» было бы свойством одного места вызова, и следующий
        вызывающий отправил бы на чужой VPS план с циклом зависимостей,
        ничего не нарушив формально.
        """
        if value is None:
            return None
        ...
        plan = RoutingPlan.from_dict(value)
...
class WorkerIntakeRequest(BaseModel):
    """Human drain/resume reason recorded in the immutable admin audit."""
