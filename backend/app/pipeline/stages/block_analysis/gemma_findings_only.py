...
SYSTEM_PROMPT_PROFILE_ASTRA_SHADOW_V3 = "astra_shadow_v3"
...
_ASTRA_SHADOW_V3_RETRIEVAL_PROTOCOL = """

## Протокол сопоставления сущностей и document_retrieval

Перед сравнением составь внутреннюю таблицу `сущность → значение в блоке → значение
в контексте`. Считай две записи одной сущностью только при точном совпадении марки
или при составном ключе из системы/этажа/помещения, типа элемента и позиции. Одинаковые
числа, общие слова и похожие названия без такого ключа не доказывают связь.

Для каждого применимого пункта дисциплинарного рецепта проверь сначала сам блок, затем
фрагменты document_retrieval. При противоречии процитируй обе стороны и укажи
evidence_kind="document_retrieval". Не сравнивай аппараты разных линий, узлы разных
систем, одноимённые помещения разных этажей или разные варианты изделия. Отсутствие
совпадающей сущности в выданных фрагментах не является доказательством отсутствия во
всём документе: в таком случае допустим только context_gap с required_context.
"""

_ASTRA_SHADOW_V3_GENERIC_RECIPE = """

## Дисциплинарный рецепт: общий

1. Выпиши все марки, позиции, номера узлов/помещений и повторённые числовые параметры.
2. Для каждой сущности сопоставь точное написание и значение во всех её появлениях.
3. Отдельно перепроверь арифметику количества, единичного значения и итога, если все
   операнды даны в переданном контексте.
"""

_ASTRA_SHADOW_V3_RECIPES = {
    "EOM": """

## Дисциплинарный рецепт: ЭОМ/ЭС

1. Для каждого QF/автомата и кода отходящей линии проверь уникальность обозначения и
   однозначность пары `аппарат ↔ линия ↔ нагрузка`; одинаковая марка у двух аппаратов
   или два кода у одной линии — отдельный кандидат.
2. Для трансформаторов тока сопоставь `Ip`, первичный/вторичный номинал и обозначение
   одного ввода. Для защиты сопоставь `Iкз(1)`, номинал/характеристику автомата и явно
   переданные пороги. Не достраивай отсутствующие технические условия из памяти.
3. Для линий СПЗ/ПЭСПЗ и иных явно пожарных нагрузок сопоставь назначение, марку кабеля
   и заявленный класс огнестойкости по одной линии.
4. Сверь марки счётчиков, аппаратов и кабелей между схемой, примечаниями и
   спецификацией только при подтверждённой общей позиции.
""",
    "KJ": """

## Дисциплинарный рецепт: КЖ/КМ

1. Построй полный перечень марок деталей и позиций арматуры/металла в блоке; для каждой
   найди точное соответствие в ведомости или спецификации. Усечённая, неоднозначная или
   отсутствующая привязка — кандидат с честным context_status.
2. Для одной позиции сопоставь диаметр/сечение, длину, шаг, количество, отметку и
   размеры во всех видах, разрезах и таблицах.
3. Пересчитай только явно заданные операции: количество × единичная длина/масса и
   сумму позиций. Приводи операнды и показанный итог.
""",
    "KM": """
...
## Дисциплинарный рецепт: АР/АИ/ТХ

1. Сопоставь название листа, тип плана/развёртки, этаж, оси, номер помещения и строку
   экспликации. Противоречие фиксируй только для одного подтверждённого вида/помещения.
2. Для дверей, проёмов, оборудования и отделки сопоставь марку, размер и исполнение на
   плане, виде/развёртке и в ведомости.
3. Проверь семантические пары `есть/нет`, назначение помещения и повторные подписи;
   похожие помещения разных этажей не объединяй.
""",
    "AI": """
...
## Дисциплинарный рецепт: ОВ/ВК

1. Для каждой системы собери ключ `система → участок/узел → оборудование → параметр`.
   Сверь номер узла/детали и ссылку со схемой, планом и выданным контекстом.
2. Для одной подтверждённой сущности сопоставь DN/диаметр, диапазон, расход, отметку,
   направление и типоразмер. Не объединяй одинаковые DN из разных систем.
3. Проверь, что диапазон или размер узла покрывает именно тот участок, к которому он
   подписан; покажи обе конфликтующие подписи.
""",
    "VK": """
...
def _astra_shadow_v3_recipe(section: str) -> str:
    from backend.app.services.common import discipline_identity as _identity
    ...
    try:
        ...
        code = None
    return _ASTRA_SHADOW_V3_RECIPES.get(
        str(code or "").upper(),
        _ASTRA_SHADOW_V3_GENERIC_RECIPE,
    )
...
def build_system_prompt(
    ...
    if extended:
        ...
        if cats:
            result += _EXTENDED_HEADER + cats + "\n"
    if prompt_profile == SYSTEM_PROMPT_PROFILE_ASTRA_SHADOW_V3:
        result += _ASTRA_SHADOW_V3_RETRIEVAL_PROTOCOL
        result += _astra_shadow_v3_recipe(section)
    if prompt_profile in {
        SYSTEM_PROMPT_PROFILE_ASTRA_SHADOW_V2,
        SYSTEM_PROMPT_PROFILE_ASTRA_SHADOW_V3,
    }:
