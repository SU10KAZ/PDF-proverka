...
#: Потолок размера одного кропа.
#:
#: Первая версия ставила 8 МиБ, рассуждая только о росте base64 (+33%). Ревью
#: указало на другое: у самого провайдера есть предел на ОДНО изображение
#: (порядка 5 МБ), и кроп между этими двумя числами уходил бы в оплаченный
#: вызов, который заведомо отвергается, — а повтор после этого запрещён.
#: Значение выбрано с запасом ПОД предел провайдера, а не под арифметику
#: кодирования.
