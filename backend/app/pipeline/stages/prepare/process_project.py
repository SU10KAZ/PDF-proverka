...
# Корневая папка проекта — автодетекция или env AUDIT_ROOT_DIR
import pathlib as _pathlib
BASE_DIR = str(_pathlib.Path(os.environ.get("AUDIT_ROOT_DIR") or os.environ.get("AUDIT_BASE_DIR") or
               _pathlib.Path(__file__).resolve().parent.parent.parent.parent.parent.parent).resolve())
...
def process(project_dir, force=False):
    ...
    try:
        from backend.app.pipeline.stages.gemma_enrichment.gemma_enrich import get_enrichment_meta
    ...
    if graph_v2:
        ...
        if enr_meta:
            try:
                from backend.app.pipeline.stages.gemma_enrichment.gemma_enrich import inject_enrichment_meta_into_graph
                from pathlib import Path as _Path
