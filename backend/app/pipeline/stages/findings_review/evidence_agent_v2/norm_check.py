...
# Корень репозитория берём из конфигурации backend (после переезда пакета в
# backend/... вычисление через parents[N] стало ненадёжным). norms/norms_api —
# top-level пакеты в корне репо, поэтому корень и norms/tools нужны в sys.path.
