...
def build_verification_prompt(md_text: str, candidates: list[dict]) -> str:
    ...
    return (
        ...
        "указано / отсутствует.\n\n"
        "Для КАЖДОГО замечания реши, глядя на ВЕСЬ документ:\n"
        "- \"present\" — заявленный элемент/данные ФАКТИЧЕСКИ ЕСТЬ в документе (в другом "
        "разделе/листе/строке) → замечание ЛОЖНОЕ.\n"
        "- \"absent\" — элемента действительно нет во всём документе → замечание верное.\n"
        "- \"not_absence\" — замечание на самом деле НЕ об отсутствии (противоречие/дубликат/"
        "неверное значение).\n\n"
