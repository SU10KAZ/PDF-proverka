...
def classify_ar_profile(text):
    ...
    # Проёмы/отверстия должны быть ТЕМОЙ плана («план проёмов», «план с маркировкой
    # отверстий»), а не попутным упоминанием: кладочные планы перечисляют Д-/ОК-марки
    # проёмов всегда и раньше утекали сюда (боевой кейс 9UN7-PPMM-9CG, АР1.2-К5 v3).
    if "ПЛАН" in u and (any(x in u for x in (
            "ПЛАН ОТВЕРСТ", "МАРКИРОВКОЙ ОТВЕРСТ", "МАРКИРОВКА ОТВЕРСТ",
            "ВЕДОМОСТЬ ОТВЕРСТ", "ВЕДОМОСТИ ОТВЕРСТ", "ВЕДОМОСТЬЮ ОТВЕРСТ"))
            or (opening and any(x in u for x in (
            "ОТВЕРСТ", "ПЛАН ПРОЕМ", "ПЛАН ПРОЁМ", "ПЛАНЫ ПРОЕМ", "ПЛАНЫ ПРОЁМ")))):
