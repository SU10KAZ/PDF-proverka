...
def detect_resume_stage(project_id: str, *, version_id: Optional[str] = None) -> dict:
    ...
    if migration_state.get("migration_required"):
        # Старый завершённый проект (есть findings/norms) — не показываем баннер
        # миграции. Новые проекты пройдут через Gemma.
        if has_03 or has_norm_checks or has_03a:
            # …но «есть какие-то артефакты» и «завершён» — РАЗНЫЕ утверждения, и
            # смешивать их нельзя. Найдено первым сетевым прогоном 11G: после
            # приёма результата воркера версия имеет 03_findings.json и НЕ имеет
            # norm_checks.json (нормативный этап — центральный, он ещё не
            # выполнялся). Кропов и `block_context_summary.json` в пакете
            # результата нет намеренно — они большие и воспроизводимые, — из-за
            # чего срабатывает признак legacy-миграции, и эта ветка отвечала
            # «Завершён» про аудит, которому осталась ровно одна стадия.
            #
            # Признаком завершённости считается `norm_checks.json` — тот же
            # артефакт, по которому судит центральный хвост
            # (`manager._run_central_tail_after_remote`: «Обязателен
            # norm_checks.json — ради него хвост и существует»). Старых
            # проектов правка не касается: у них этот файл есть.
