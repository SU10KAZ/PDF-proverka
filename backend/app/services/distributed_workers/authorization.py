...
def _parse_list(raw: str, *, env_name: str, errors: list[str]) -> list[str]:
    ...
    for chunk in text.split(_SEPARATORS[0]):
        ...
        if "*" in subject or "?" in subject:
            ...
            #
            # Само значение в текст ошибки НЕ подставляется. Диагностика уходит
            # в `GET /api/workers/status` и `GET /api/workers/me`, а их читает
            # любой вошедший в портал — в том числе человек без роли в
            # подсистеме. Одна опечатка в `.env` («andrey*») выдала бы ему имя
            # администратора, то есть готовую цель для подбора пароля.
            # Администратор видит, В КАКОЙ переменной ошибка и какого рода;
            # само содержимое переменной он читает в `.env`, а не через API.
            errors.append(f"{env_name}: шаблоны и подстановки запрещены")
            continue
