...
def to_view(
    ...
    *,
    ...
    return {
        ...
        "intake_enabled": bool(row.get("intake_enabled", 0)),
        "intake_updated_at": row.get("intake_updated_at"),
        "intake_updated_by": row.get("intake_updated_by"),
        "intake_reason": row.get("intake_reason") or "",
...
def can_receive_jobs(row: dict[str, Any]) -> tuple[bool, str]:
    ...
    if not bool(row.get("intake_enabled", 0)):
        return False, "Приём новых заданий остановлен оператором"
