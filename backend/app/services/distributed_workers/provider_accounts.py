...
QUOTA_REASON_CODES: tuple[str, ...] = (
    ...
    # Отдельный код, потому что действие оператора уникально: не логиниться
    # заново и не ждать сброса лимита, а получить доступ у администратора
    # организации либо переавторизовать воркер на другую учётную запись.
