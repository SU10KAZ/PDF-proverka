...
RELATIVIZE_KEYS: frozenset[str] = frozenset(
    {
        ...
        "image_path",
        "md_path",
        # `md_file` — то же самое, что `md_path`, но под другим именем: его
        # пишет `md_prescan` в `01_text_prescan.json`. Найдено ПЕРВЫМ сетевым
        # прогоном 11G: контракт назвал `md_path` и не назвал `md_file`, и
        # пакет результата целиком отвергался с «поле с абсолютным путём не
        # описано контрактом». Отказ был правильным — недоставало именно
        # строки в этой таблице. До 11F дефект не проявлялся: предсканирование
        # MD молча не выполнялось из-за неверного корня версии (b2247d8a), то
        # есть файла с этим полем просто не возникало.
        "md_file",
        "pdf_path",
        "source_pdf",
        "source_md",
