...
def set_worker_intake(
    ...
    *,
    ...
    """Atomically change the human-owned intake gate.

    The previous row is returned for immutable audit logging.  Heartbeats do
    not write these columns, so a machine cannot silently resume itself after
    an operator drain.
    """
    now = time.time()
    with database.write_txn(settings) as conn:
        ...
        previous = dict(row)
        conn.execute(
            "UPDATE workers SET intake_enabled=?, intake_updated_at=?, "
            "intake_updated_by=?, intake_reason=?, updated_at=? WHERE worker_id=?",
            (1 if enabled else 0, now, actor, reason[:500], now, worker_id),
        )
...
def set_worker_intake_with_audit(
    worker_id: str,
    *,
    enabled: bool,
    actor_id: str,
    ...
    """Atomically persist the human intake decision and its evidence row."""
    now = time.time()
    action_id = new_id("act", 12)
    action_type = "worker_intake_resumed" if enabled else "worker_drained"
    with database.write_txn(settings) as conn:
        row = conn.execute(
            "SELECT * FROM workers WHERE worker_id = ?", (worker_id,)
        ).fetchone()
        ...
        conn.execute(
            "UPDATE workers SET intake_enabled=?, intake_updated_at=?, "
            "intake_updated_by=?, intake_reason=?, updated_at=? WHERE worker_id=?",
            (1 if enabled else 0, now, actor_id, reason[:500], now, worker_id),
        )
        ...
        conn.execute(
            "INSERT INTO worker_admin_actions (action_id,actor_id,actor_display_name,"
            "action_type,worker_id,previous_state_json,requested_state_json,reason,"
            "request_id,source_ip,user_agent,created_at,result_status,result_json,"
            "actor_role,permission) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (
                action_id,
                actor_id,
                actor_display_name,
                ...
                worker_id,
                json.dumps(
                    {"intake_enabled": bool(previous.get("intake_enabled", 0))},
                    ensure_ascii=False,
                ),
                json.dumps({"intake_enabled": enabled}, ensure_ascii=False),
                reason[:500],
                request_id,
                source_ip,
                user_agent,
                ...
                "ok",
                json.dumps(
                    {"intake_enabled": bool(current.get("intake_enabled", 0))},
                    ensure_ascii=False,
                ),
                actor_role,
                permission,
        ...
        action_row = conn.execute(
            "SELECT * FROM worker_admin_actions WHERE action_id = ?", (action_id,)
        ).fetchone()
    return previous, current, dict(action_row)
