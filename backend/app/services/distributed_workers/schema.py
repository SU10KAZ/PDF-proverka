...
# ─────────────────────────────────────────────────────────────────────────────
# Миграция 8 (этап 11, по итогам адверсариальной проверки): ПРОИСХОЖДЕНИЕ
# привязки провайдера к учётной записи.
#
# Привязку можно получить двумя путями, и они не равны по доверию:
#   * `operator` — человек нажал кнопку либо вызвал API. Доверенный источник.
#   * `worker`   — воркер объявил группу сам, переменной окружения на своей
#                  машине. Это документированный и штатный механизм, но
#                  источник полу-доверенный: воркер объявляет что угодно.
#
# Без различения этих двух случаев возникала дыра: воркер, впервые вышедший на
# связь и объявивший ЧУЖУЮ группу, попадал в ту же учётную запись и мог
# оказаться выбранным снимком при сведении — то есть показать оператору
# выдуманный остаток по аккаунту, к которому отношения не имеет.
#
# Колонка не запрещает самопривязку (она штатная), а делает её видимой и
# младшей по приоритету: при сведении группы снимок воркера, привязанного
# ОПЕРАТОРОМ, идёт впереди самопривязанного.
_MIGRATION_8 = """
ALTER TABLE worker_provider_states
