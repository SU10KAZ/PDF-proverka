...
# Код дисциплины в начале project_id: «AR/», «EOM/», «PS/». Только латиница и
# цифры и не длиннее шести знаков — так слэш внутри самого имени комплекта
# (кириллица, пробелы, длинные сегменты) не будет принят за префикс.
_DISCIPLINE_PREFIX_RE = re.compile(r"[A-Za-z][A-Za-z0-9_-]{0,5}")
...
def canonical_project(source_project: str, section: str = "") -> str:
    """Каноническое имя проекта: убрать дисциплинарный префикс «<КОД>/».

    Один и тот же проект попадает в decisions_log под двумя написаниями
    `source_project`: с префиксом (`OV/13АВ-РД-ОВ2-К1 V1`) и «голым»
    (`13АВ-РД-ОВ2-К1 V1`). Первую форму пишет загрузка решений из Excel (там
    project_id берётся из скрытой колонки файла), вторую — сохранение из
    интерфейса (имя из адреса). Для графика это ОДИН проект — иначе он двоится
    в одном дне.

    Срезаем ЛЮБОЙ ведущий сегмент, похожий на код дисциплины, а не только
    совпадающий с `section`: у проекта из папки заказчика `.../PS/ПД-…-ПС-1_V1`
    префикс `PS`, а дисциплина документа `SS` — сверка с section не срабатывала,
    и Репников получал в графике четыре плитки вместо двух (06.08.2026).

    Разные дисциплины с одинаковым «голым» именем НЕ сольются: ключ группы
    включает object_id и section.
    """
    pr = (source_project or "").strip()
    if "/" not in pr:
        return pr
    head, _, tail = pr.partition("/")
    # Код дисциплины: короткий токен из латиницы/цифр («AR», «EOM», «PS»).
    # Кириллица и пробелы — признак того, что слэш относится к самому имени.
    if tail and _DISCIPLINE_PREFIX_RE.fullmatch(head):
        return tail.strip()
    return pr
