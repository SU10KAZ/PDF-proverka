...
from backend.app.models.discussion import (
    Discussion,
    DiscussionMessage,
    DiscussionListItem,
)
...
async def _run_cli_chat(prompt_text: str) -> LLMResult:
    ...
    from backend.app.core.config import get_claude_model
...
async def _stream_cli_chat(prompt_text: str) -> AsyncGenerator[dict, None]:
    ...
    from backend.app.services.common.process_runner import run_command_stream
    from backend.app.core.config import get_claude_model
...
async def send_chat_message_stream(
    ...
    if _is_cli_model(model):
        ...
        # OpenRouter стриминг
        from backend.app.services.llm.llm_runner import run_llm_stream
        messages = _build_messages(discussion, context_text, images, user_message, is_first)
