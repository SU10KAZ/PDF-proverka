...
1. Сначала используй компактный индекс SHEET CONTEXT: секции,
   функциональные узлы, таблицы, графовые связи, уже доказанные FAST
   изменения и внутренние противоречия. Полные записи текущей задачи находятся
   в FOCUSED EVIDENCE; индекс не является доказательством отсутствия.
...
6. Для GRAPH_RELATION называй реально существующую связь. Для VALUE копируй
   ...
   Для FORMATTING_ONLY докажи равенство одной и той же нормализованной
   инженерной величины: например, два отдельных VALUE claim с attribute=count
   и одинаковым числом из левой и правой кабельной записи. Две разные полные
   строки не являются доказательством равенства.
...
10. RESOLVED недопустим с INS UFFICIENT_EVIDENCE, UNKNOWN confidence, пустыми
    ...
    Метки внутри FOCUS:TEXT (например L1/R7) — части записи, а не evidence_ref;
    в subject_ref и evidence_refs используй только полный разрешённый
    FOCUS:TEXT:<task_id>.
