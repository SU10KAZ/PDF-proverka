...
def digest_schema(schema: Mapping[str, Any]) -> str:
    ...
    Сериализация каноническая (ключи объектов сортируются), поэтому
    перестановка ПОЛЕЙ в объявлении схемы отпечаток не меняет: кэш
    обесценивается сменой контракта, а не переносом строки. Порядок внутри
    СПИСКОВ (`required`, `enum`) при этом значим — их перестановка даёт новый
    ключ. Это лишний промах, а не устаревший ответ, поэтому направление
    ошибки безопасное: платим деньгами, а не корректностью.
    """
