...
PROMPT_VERSION = "stage-comparison-ai-identity.v1"
...
4. Подпись — не доказательство сама по себе. «ШУ-ХЦ» слева и «ВРУ-ХЦ» справа
   ...
   а) shared_identity — дословная подстрока, которая есть В ОБЕИХ строках
      (например обозначение, напечатанное в ряду над колонкой);
   б) арифметика: сумма расчётных мощностей фидеров обязана сойтись со
      ...
      Система пересчитает сумму и отклонит ответ, если она не сходится.
