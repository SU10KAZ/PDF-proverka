...
(function(){
  ...
  const OBJECT=qs.get('object')||'4f3e5916';
  const PAIR_PARAM=qs.get('pair')||'';
  const API='/api/human-mapping/objects/'+encodeURIComponent(OBJECT);
  const PAIR_KEYS={'A':'ad0a31a342a666082f2ef66a','B':'caea6d2810c334ec0368de8e','ad0a31a342a666082f2ef66a':'A','caea6d2810c334ec0368de8e':'B'};
  if(PAIR_PARAM && PAIR_KEYS[PAIR_PARAM]){ /* pair letter or key */
    if(PAIR_PARAM==='A'||PAIR_PARAM==='B'){ window.__HM_PAIR__=PAIR_PARAM; }
    else { window.__HM_PAIR__=PAIR_KEYS[PAIR_PARAM]; }
  }
  window.__HM_API__=API;
  window.__HM_OBJECT__=OBJECT;
  const _fetch=window.fetch.bind(window);
  window.fetch=function(url, opts){
    let u=String(url);
    if(u.startsWith('/data/UI_DATA_PAIR_')){
      const letter=u.replace('/data/UI_DATA_PAIR_','').replace('.json','');
      u=API+'/pairs/'+encodeURIComponent(letter)+'/ui-data';
    } else if(u.startsWith('/reviews')){
      const p=new URL(u, location.origin).searchParams.get('pair')||window.__HM_PAIR__||'A';
      u=API+'/pairs/'+encodeURIComponent(p)+'/reviews';
    } else if(u.startsWith('/block_links')){
      const p=new URL(u, location.origin).searchParams.get('pair')||window.__HM_PAIR__||'A';
      u=API+'/pairs/'+encodeURIComponent(p)+'/block-links';
    } else if(u.startsWith('/assets/')){
      u=API+'/assets/'+u.slice('/assets/'.length);
    }
...
<script>
let pair=(window.__HM_PAIR__||'A'),queue='UNREVIEWED',data,history=[],linkEvents=[],selected={OLD:[],NEW:[]},panMode=null,showAll=false,editMode=false,selectedLinkId=null,reassignSide=null,view={OLD:{x:0,y:0,z:1,locked:false},NEW:{x:0,y:0,z:1,locked:false}};const $=s=>document.querySelector(s),esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
...
async def post_review(
    ...
    storage.append_review(object_id, pair_key, row, smoke=smoke)
    return row
...
async def post_block_link(
    ...
    events = storage.list_block_links(object_id, pair_key, smoke=smoke)
    try:
    ...
    storage.append_block_link(object_id, pair_key, row, smoke=smoke)
    return row
