...
async def get_page_svg(
    ...
    Единственный источник страниц во вкладке «Связь блоков»: растровый режим
    и тумблер переключения убраны из интерфейса. `page-image` остаётся живым
    endpoint'ом, но фронтенд его больше не зовёт и на растр не откатывается.
    """
    try:
