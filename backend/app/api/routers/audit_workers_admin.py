...
async def _set_worker_intake(
    worker_id: str,
    *,
    enabled: bool,
    payload: WorkerIntakeRequest,
    request: Request,
) -> dict[str, Any]:
    settings = _settings_or_404()
    _require_intent_header(request)
    meta = _audit_meta(request, permission=authorization.PERM_OPERATE)
    actor = authorization.actor_of(request)
    try:
        _previous, current, _action = await database.run_db(
        ...
        raise HTTPException(status_code=404, detail="Воркер не найден.") from exc
    ...
    return {"worker": await _worker_view(current, settings)}


@router.post("/{worker_id}/drain")
async def drain_worker_intake(
    worker_id: str,
    payload: WorkerIntakeRequest,
    request: Request,
    actor: Actor = Depends(require_operator),
) -> dict[str, Any]:
    """Human, audit-logged stop of NEW intake; active jobs are untouched."""
    del actor
    _require_intent_header(request)
    return await _set_worker_intake(
        worker_id, enabled=False, payload=payload, request=request
    )


@router.post("/{worker_id}/resume-intake")
async def resume_worker_intake(
    worker_id: str,
    payload: WorkerIntakeRequest,
    request: Request,
    actor: Actor = Depends(require_operator),
) -> dict[str, Any]:
    """Human, audit-logged enablement of NEW intake for one worker."""
    del actor
    _require_intent_header(request)
    return await _set_worker_intake(
        worker_id, enabled=True, payload=payload, request=request
    )
