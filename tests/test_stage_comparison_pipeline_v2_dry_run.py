...
def test_1_creates_all_8_artifacts(dry_run_result):
    _, out = dry_run_result
    expected = [
        "left_normalized_document_model.json", "right_normalized_document_model.json",
        "block_matching_report.json", "entity_extraction_report.json",
...
def test_4_manifest_has_sha256_and_sizes(dry_run_result):
    ...
    assert len(existing) == 7  # всё, кроме самого манифеста
    for e in existing:
