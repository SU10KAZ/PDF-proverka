...
# 2. Статусы
# --------------------------------------------------------------------------
def test_ровно_пять_статусов():
    """Защёлка перечня. Пятый статус добавлен осознанно вместе с разделом
    «Уточнено ИИ и проверено правилами»: происхождение находки инженер обязан
    видеть, а название модели и уровень рассуждения — нет."""
    assert pr.STATUSES == (
...
def test_статус_не_называет_модель():
    """Инженеру говорят, чем находка доказана, а не кем посчитана."""
    forbidden = ("gpt", "codex", "claude", "opus", "sonnet", "reasoning", "low")
    for status in pr.STATUSES:
        lowered = status.lower()
        assert not any(word in lowered for word in forbidden), status
