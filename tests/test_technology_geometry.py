...
def test_tx_corpus(case):
 ...
 g=json.loads((OUT/f"{case['block_id']}.structure.json").read_text());assert g["profile_id"]==case["profile_id"] and evaluate_tx_gate(g)["use"]
 text=render_tx_markdown(g);assert "Эталонная текстовая разметка ТХ" in text and g["profile_id"] not in text and g["validation"]["subtype"] not in text
...
def test_tx_coverage_and_semantics():
 ...
 assert len(gs)==35 and {g["profile_id"] for g in gs}==set(ALL_TX_PROFILES)-{"tx_waste_plan"} and len({c["subtype"] for c in cases()})==9
