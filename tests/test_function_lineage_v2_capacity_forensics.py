...
def test_latent_false_conflicts_exist_and_are_confined_to_proven_classes(artifact: dict) -> None:
    ...
    assert latent["counts"]["D_FRAGMENTATION_DEFECT"] == 151
    assert latent["counts"]["C_TASK_DUPLICATION"] == 0
    assert latent["counts"]["G_UNKNOWN"] == 0
    # Unrelated scopes never produce a hierarchical duplicate, and related
    ...
    assert by_relation["OVERLAP"]["A_TRUE_FUNCTION_FRAGMENT_CONFLICT"] == 0
