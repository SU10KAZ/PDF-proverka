...
describe('слой Б: видимый текст раздела', () => {
    ...
    it('печатает адрес изменения только там, где он и есть подпись', () => {
        // Колонка «ID» итогового отчёта — сознательное исключение: инженеру
        // нужно чем-то назвать строку в переписке. Всё остальное показывает
        // объект, а адрес прячет в диагностику.
        const printed = mustacheExpressions(WITHOUT_DIAGNOSTICS)
            .filter(expression => /^\s*row\.target_id\s*$/.test(expression));
        expect(printed).toEqual(['row.target_id']);
    });
