...
*{box-sizing:border-box}body{margin:0;background:#f4f6f8;color:#162431;font:14px system-ui,sans-serif}header,.bar,.main{max-width:1800px;margin:auto;padding:14px 2%}header{background:white;max-width:none;padding-left:calc((100% - 1800px)/2 + 2%);border-bottom:1px solid #d5dce2}h1{font-size:20px;margin:0 0 4px}.note{color:#596575}.bar{display:flex;gap:7px;align-items:center;flex-wrap:wrap}.bar button,.decision button{padding:7px 10px;border:1px solid #aebbc6;background:white;border-radius:5px;cursor:pointer}.bar button.active{background:#d7f0ed;border-color:#087c72}.progress{margin-left:auto;color:#52616d}.card{background:white;border:1px solid #d5dce2;border-radius:8px;padding:13px}.title{display:flex;justify-content:space-between;gap:12px}.pages{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px}.side h2{font-size:15px;margin:0 0 7px}.page{position:relative;background:#e5eaee;margin-bottom:10px;overflow:hidden}.page img{display:block;width:100%;height:auto}.block{position:absolute;border:2px solid #1677c8;background:#1677c822;cursor:pointer}.block.TABLE{border-color:#e07b00;background:#e07b0022}.block.GRAPHIC{border-color:#8d43b6;background:#8d43b622}.block.selected{outline:3px solid #e62d86;background:#e62d8640}.link{padding:8px;background:#edf7f6;border-left:4px solid #087c72;margin:8px 0}.decision{display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-top:12px}.decision textarea{min-width:260px;min-height:36px;padding:6px}.detail{margin-top:10px;background:#f7f9fa;padding:10px}.detail pre{white-space:pre-wrap;max-height:240px;overflow:auto}.detail table{border-collapse:collapse;width:100%}.detail td,.detail th{border:1px solid #ccd5dc;padding:4px}.crop{max-width:100%;max-height:300px;cursor:zoom-in}.manual{background:#fff7e7;border-left:4px solid #e07b00;padding:8px;margin-top:10px}@media(max-width:900px){.pages{grid-template-columns:1fr}.progress{margin-left:0}}</style>
...
let pair='A',queue='UNREVIEWED',data,history=[],selected={OLD:[],NEW:[]};const $=s=>document.querySelector(s),esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
...
function latest(r){let x=history.filter(h=>h.region_id===r.id);return x.length?x[x.length-1]:null}function stat(r){return latest(r)?.status||'UNREVIEWED'}
function overlay(p,b){let [x,y,w,h]=b.bbox;return `<button class="block ${b.type} ${selected[p.side].includes(b.id)?'selected':''}" title="${esc(b.id)} · ${b.type}" data-side="${p.side}" data-id="${b.id}" style="left:${x*100}%;top:${y*100}%;width:${(w-x)*100}%;height:${(h-y)*100}%"></button>`}
function page(p){return `<div class=page><img src="/${p.image}">${p.blocks.map(b=>overlay(p,b)).join('')}</div>`}
...
function render(){for(let x of ['A','B'])$('#'+x).classList.toggle('active',pair===x);let reviewed=data.regions.filter(r=>stat(r)!=='UNREVIEWED').length;$('#progress').textContent=`Проверено ${reviewed} / ${data.regions.length}`;let r=data.regions.find(x=>stat(x)===queue)||data.regions[0];if(!r){$('#app').innerHTML='<p>В этой очереди нет связей.</p>';return}let l=latest(r);$('#app').innerHTML=`<section class=card><div class=title><div><b>${r.id}</b> · ${esc(r.domain)}<p>${esc(r.scope)}</p></div><small>AI-proposed link · confidence ${r.confidence}</small></div><div class=link>OLD blocks (${r.old_blocks.map(x=>esc(x.id)).join(', ')}) ↔ NEW blocks (${r.new_blocks.map(x=>esc(x.id)).join(', ')})</div><div class=pages><div class=side><h2>OLD</h2>${r.pages.OLD.map(p=>page({...p,side:'OLD'})).join('')}</div><div class=side><h2>NEW</h2>${r.pages.NEW.map(p=>page({...p,side:'NEW'})).join('')}</div></div><div class=manual>Выберите один или несколько блоков слева и справа, чтобы создать ручную связь. Выделены: OLD ${selected.OLD.length}, NEW ${selected.NEW.length}.</div><div class=decision><textarea id=comment placeholder="Комментарий (необязательно)"></textarea><button data-status=HUMAN_CONFIRMED>ПОДТВЕРДИТЬ</button><button data-status=HUMAN_REJECTED>ОТКЛОНИТЬ</button><button data-status=HUMAN_UNCERTAIN>НЕ УВЕРЕН</button></div>${l?`<p>Последнее решение: <b>${l.status}</b> · ${esc(l.timestamp)}</p>`:''}<div id=details></div></section>`;document.querySelectorAll('.block').forEach(b=>b.onclick=()=>{let s=b.dataset.side,id=b.dataset.id;selected[s]=selected[s].includes(id)?selected[s].filter(x=>x!==id):[...selected[s],id];$('#details').innerHTML=showDetail(s,id,r);render()});document.querySelectorAll('[data-status]').forEach(b=>b.onclick=()=>save(r,b.dataset.status))}
...
$('#A').onclick=()=>{pair='A';load()};$('#B').onclick=()=>{pair='B';load()};document.querySelectorAll('[data-q]').forEach(b=>b.onclick=()=>{queue=b.dataset.q;render()});load()</script>'''
