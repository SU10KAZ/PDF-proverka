...
def deploy_lock(
    ...
    *,
    ...
    `reentrant` — НЕ удобство, а узкое разрешение для одного случая:
    обязательная блокировка стоит и у команды выкатки, и у мутирующего
    примитива внутри неё. Снаружи она обязана быть, иначе прямой вызов
    примитива (как в smoke-сценарии) обходил бы защиту; изнутри без этого
    флага штатная выкатка не смогла бы сделать шаг, который сама же начала.
    По умолчанию флаг ВЫКЛЮЧЕН: вторая выкатка обязана получать отказ, даже
    если она почему-то запущена в том же процессе.
    """
    path = lock_path(component, instance=instance, lock_dir=lock_dir)
    if reentrant and str(path) in _HELD_FDS:
        # Замок уже у ЭТОГО процесса. Граница честная: повторный вход
        # рассчитан на однопоточные сценарии выкатки.
        yield path
        return
    ...
    try:
        ...
        # Запись убирается ТОЛЬКО если её сделал этот вызов. Вложенная попытка
        # взять тот же замок отказывает по contention — и, снося чужую запись,
        # лишала бы внешнего держателя возможности передать замок в `exec`.
        if registered:
