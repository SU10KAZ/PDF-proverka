...
REQUIRED_PATHS = (
    ...
    ".mcp.json",
    "norms/runtime.py",
    "norms/tools/mcp_launcher.py",
    "norms/tools/mcp_server.py",
    "norms/tools/norms_api.py",
    "scripts/release_staging.py",
...
def build(
    *,
    ...
    # НИ ОДИН из этих путей не имеет права быть символической ссылкой.
    #
    # Это не педантизм, а разбор инцидента 19.08.2026. Сборка велась с
    # `--releases-dir`, где донор был ССЫЛКОЙ на боевой релиз центра. `cp -a`
    # копирует ссылку как ссылку, поэтому staging оказался не копией, а вторым
    # именем боевого каталога — и дальнейшие шаги сборки (переименование `app`
    # в `app.previous`, распаковка нового дерева, удаление предыдущего)
    # выполнились ПРЯМО В РАБОТАЮЩЕМ РЕЛИЗЕ. Каталог `ui-real-9b1dbedd` стал
    # содержать дерево другого коммита, а `current` продолжал на него
    # указывать: любой рестарт поднял бы прод не тем кодом.
    #
    # Защита стоит до первого действия: после `cp -a` отличить «скопировали
    # дерево» от «получили второе имя чужого каталога» уже нечем.
    for label, path in (
        ("каталог релизов", releases_dir),
        ("релиз-донор", releases_dir / base_release),
    ):
        if path.is_symlink():
            raise SystemExit(
                f"{label} — символическая ссылка: {path} → {os.readlink(path)}. "
                "Сборка по ссылке ведётся в чужом каталоге (возможно боевом) — "
                "укажите настоящий каталог"
            )
...
def _build(tmp, *, base, final, release_id, commit, parent, tree, parent_tree,
    ...
    manifest.update({
        ...
        "norms_runtime": {
            ...
            "bundled": False,
            "required": True,
            "environment_override": "NORMS_TOOLS_PATH",
            "default_layout": "<auditmanager-root>/shared/norms/tools",
            "deploy_precheck": "scripts/deploy_center_release.py",
        },
