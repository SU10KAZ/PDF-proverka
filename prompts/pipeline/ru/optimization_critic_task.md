...
- Проверь, является ли пункт самостоятельной оптимизацией, а не пересказом уже существующего замечания F/T. Если `proposed` только исправляет размер, маркировку, ГОСТ, состав листа или обязательное нормативное требование и не даёт отдельного эффекта по стоимости, сроку, унификации или жизненному циклу → `verdict: "too_vague"` с пояснением «повторяет finding, самостоятельная оптимизация отсутствует».
- Ссылка на finding сама по себе не является доказательством экономического эффекта. Для `pass` должен оставаться независимый предмет оптимизации: аналог, сокращение типоразмеров/операций, заводской модуль, изменение технологии монтажа или измеримый lifecycle-эффект.
