...
# Webapp.main:app остался legacy. Cron-watchdog запускает этот скрипт при
# падении, поэтому он должен поднимать именно production-backend, иначе после
# любой ошибки legacy webapp займёт 8081 и serverCaps.v2AuditSupported=false
# для UI станет фактически правдой.
