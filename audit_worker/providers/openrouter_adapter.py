...
def resolve_base_url(env: Optional[dict[str, str]] = None) -> str:
    ...
    if not stubbed_endpoints_declared(env=source):
        ...
        raise OpenRouterEndpointError(
            f"{BASE_URL_ENV}={raw!r} указывает не на официальный шлюз "
            f"{OFFICIAL_HOST!r} по HTTPS. Такой адрес принимается только при "
            f"объявленном {STUBBED_ENDPOINTS_ENV}=true, и объявление уезжает "
            "в heartbeat: подменить боевой шлюз молча нельзя"
        )
    return raw
