...
Инварианты слоя (§9 и §28 задания), выраженные в коде:
  ...
  I-P8. Подпроцесс НЕ получает stdin вызывающего. Когда своего ввода у него
        нет, на его месте `/dev/null`, а не унаследованный дескриптор. Оба
        CLI в неинтерактивном режиме читают промпт из stdin, если он не пуст:
        `claude -p` — напрямую, `codex exec` — добавляя `<stdin>`-блок к
        промпту. Унаследованный stdin означал бы, что содержимое канала
        вызывающего становится промптом. Это не теория: при подготовке этапа
        11b `claude`, запущенный из скрипта, который bash читал со стандартного
        ввода, прочитал остаток этого скрипта как промпт и выполнил
        незапланированный запрос к модели.
"""
...
class ProviderAdapter(abc.ABC):
    ...
    def structured_inference(
        ...
        *,
        ...
        `model` (этап 11D) — ТОЧНЫЙ идентификатор из ЛОКАЛЬНОЙ политики воркера,
        который адаптер обязан предъявить CLI явно. Он не является данными
        задания: центр присылает логическую способность, а строку подставляет
        файл администратора машины (см. `model_policy`), поэтому инвариант I-P5
        («в argv только то, что не пришло извне») остаётся в силе.

        `accepted_reported_models` — идентификаторы, которые CLI имеет право
        назвать фактически применёнными для той же модели. Несовпадение —
        отказ (fail closed), а не предупреждение: молча ответившая другая
        модель даёт результат, который нельзя сравнивать ни с историей, ни с
        соседним прогоном.
        ...
        *,
        ...
        try:
            proc = subprocess.Popen(
                ...
                env=env,
                # I-P8: свой ввод есть — даём канал; своего нет — даём
                # `/dev/null`, а не унаследованный дескриптор. Пустой канал
                # тоже не годится: `codex exec` считает «stdin передан» и
                # дописывает к промпту `<stdin>`-блок, то есть контрольный
                # запрос перестал бы быть побайтово одинаковым.
