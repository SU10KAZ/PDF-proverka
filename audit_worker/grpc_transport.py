...
class GrpcStreamControlTransport:
    ...
    def _hello(self, epoch: int) -> stream_pb.AgentHello:
        ...
        with self._heartbeat_lock:
            # Событие, накопленное в ПРОШЛОМ соединении, здесь и умирает.
            # Оно описывает более старый снимок, чем тот, что уезжает в этом
            # Hello, а отправляется сразу после CenterHello — то есть затирало
            # бы свежее состояние на центре. Исправить это следующим heartbeat
            # уже нельзя: опорная точка к тому моменту равна свежему снимку, и
            # изменения транспорт не увидит. Молчание было бы вечным.
            self._latest_capabilities = None
