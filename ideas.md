...
### Идея 8 — Замена модели Stage 02 (GPT-5.4/low → GPT-5.5 или medium)
**Дата:** 2026-05-04
**Формулировка (как сказал пользователь):**
> сохрани в идеях это сравнение чтоб в будущем рассмотреть замену модели для проверки блоков

**Контекст / зачем:**
- Текущий продакшн: `openai/gpt-5.4`, `reasoning=low`, `temperature=0.2`, `max_tokens=16000`
- Провели бенчмарк на 10 блоках ЭМ-К3 (Alia ASTERUS, ЭОМ) — все 4 варианта в идентичных условиях

**Результаты бенчмарка (свежие прогоны, одинаковый payload):**

| Вариант | Findings | Стоимость (10 блоков) | Avg/блок |
|---|---|---|---|
| GPT-5.4 / low | 22 | $0.21 | 23 с |
| GPT-5.4 / medium | 31 | $0.55 | 57 с |
| GPT-5.5 / low | 18 | $0.23 | 9 с |
| GPT-5.5 / medium | 24 | $0.76 | 46 с |

**Ключевые выводы:**
- **GPT-5.4/medium** — лидер по количеству и качеству: +41% findings vs low, стабилен (±1 при повторном прогоне). Лучше видит противоречия план↔таблица↔спецификация, мисматчи обозначений. Дороже в 2.6×.
- **GPT-5.5/low** — почти та же стоимость что и 5.4/low (+9%), но на 18% меньше findings. В 2.4× быстрее. Нашла уникальное КРИТИЧЕСКОЕ: сечение проводников 2,5 мм² вместо 4 мм² (ПУЭ п.1.5.19) — то что 5.4 полностью пропустила.
- **GPT-5.5/medium** — дороже 5.4/medium в 1.4×, findings меньше (24 vs 31). Лучше видит функциональные нарушения (fail-safe, огнестойкость кабелей МДУ, технические параметры).
- **Старый продакшн-прогон GPT-5.4/low (29.04.2026) показал 33 findings** — оказался случайно удачным, при повторном прогоне в идентичных условиях даёт 22.

**Сильные стороны по типу замечаний:**
- Логические противоречия в документации → **GPT-5.4/medium**
- Технические параметры (сечения, ПКС) → **GPT-5.5**
- Пожарная безопасность, fail-safe → **GPT-5.5/medium**
- Нормативные ссылки с номерами пунктов → **GPT-5.5** (реже, но точнее)

**Скрипты бенчмарка:**
- `scripts/benchmark_all_fresh.py` — все 4 варианта параллельно, идентичные условия
- `scripts/benchmark_all_fresh_results/` — результаты по блокам + summary.json

**Что попробовали:**
- Прогнали все 4 варианта на 10 репрезентативных блоках (разные типы чертежей: однолинейные схемы, планы трасс, узлы, спецификации)
- Верифицировали что старые данные GPT-5.4/low были в других условиях (продакшн vs бенчмарк)
