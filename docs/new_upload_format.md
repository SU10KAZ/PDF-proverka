...
4. **result.json из blocks.json + кэш кропов**: прямой детерминированный
   ...
   **Заглушка «локальные векторные кропы» (16.07.2026, до перевода кропов
   портала на вектор):** по умолчанию кропы НЕ скачиваются, а вырезаются из
   исходного PDF комплекта по page_index+coords_norm (`pdf_crop.py`:
   одностраничный PDF + set_cropbox; derotation_matrix для /Rotate; якорь
   сдвига = верх-лево ВИДИМОЙ области CropBox∩MediaBox — getter cropbox
   отдаёт сырой /CropBox, а mediabox бывает от −w/2 в CAD-экспортах;
   гард на /Rotate не кратный 90 → PdfCropError; запись атомарна
   tmp+os.replace) — кроп сохраняет вектор-слой (pdftotext/вектограф
   работают дословно, гомоглифов нет). Пофайловый фолбэк на скачивание по
   crop_url (в т.ч. без PyMuPDF в окружении); откат целиком:
   `AUDIT_CROP_CACHE_SOURCE=download`. Кэш кропов исключён из
   `_v2_input_names` (не пользовательский исходник). Ограничения: полигон
   режется по bbox без маски; кроп страницы-скана весит как страница
   (viewport-кроп) и текста не содержит. Валидация: АР 29/29 и ОВ0-1
   552/552 блоков, медиана совпадения токенов с MD 96–97%, ~1-10 с на
   552 блока; ревью 4 линзы + верификация, 11 находок исправлено.
