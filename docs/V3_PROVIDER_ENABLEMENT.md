...
Объект `4f3e5916` отдаёт запечатанный снимок (121 карточка) через мост
просмотра: сессия просмотра строится из реестра пар самого снимка, и каждая
карточка привязана к своей паре (84 у АР1, 37 у ИОС4.2). Как только у объекта
появится опубликованный живой прогон V3, он вытесняет мост: ответ переходит в
режим `PRODUCTION_V3` с пометкой `snapshot_viewer = SUPERSEDED_BY_LIVE_V3`, и
интерфейс работает в реальной сессии объекта.
