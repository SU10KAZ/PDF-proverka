...
backend/app/pipeline/stages/block_grounding/ar_ceiling_lighting/
  ...
  symbols.py       классификация по эталонам; снятие кружков-оформления подписей групп;
                   составные потолочные маркеры; раскрой «слипшихся» числовых спанов
  ...
  render_md_audit.py   AUDIT-контекст + секционный build_audit_context (режим «Аудит», UI по умолчанию)
...
tests/test_ar_ceiling_lighting_profile.py        79 тестов (синтетика + регресс на эталоне)
...
|---|---|---|---|
| `markdown` (full) | 87 026 симв. / 1287 строк | диагностика и приёмка | артефакт + API, **в UI не показывается** |
| `markdown_compact` | 31 424 симв. / 435 строк | подробное поквартирное описание | режим «Подробно» |
| `markdown_audit` | 9 110 симв. / 88 строк | высокосигнальный контекст для поиска замечаний | режим **«Аудит» — по умолчанию** |
...
1. **Подключение audit к Stage 01/02 за отдельным флагом.** Сейчас профиль
   ...
   контрольной выборке до включения.
2. **Листы 005–007/010** (`partial`, нет квартирных марок) — другая
   структура листа; нужен отдельный подпрофиль или расширение гейта.
3. **Rotation ≠ 0** (лист 008) — нормализация поворота в `coords.py`.
4. **16 неразрешённых символов на листе 003** — слипаются с красными
   шлейфами пожарной сигнализации в один кластер; нужен разрез кластера
   по слою/цвету до классификации.
5. **Границы потолочных зон** при нескольких марках в одном помещении —
   в вектор-слое не выделены, зоны остаются `unresolved`.
6. **Косметика легенды**: одна строка задваивает хвост
   («дана от отметки чистого пола.дана от отметки чистого пола») —
   склейка переносов в `legend.py`.
7. **Раскатка на другие профили АР** (`ar_finish_plan`, `ar_wall_elevation`,
   `ar_detail`) по той же схеме: легенда листа → сигнатуры → граф →
   три представления.
8. **Обобщение в `graphic_primitives/`** — см. `docs/graphic_anchors/`
