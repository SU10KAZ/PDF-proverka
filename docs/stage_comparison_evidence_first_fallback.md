...
- **scope-only разделы** — раздел существует только с одной стороны → ОДИН
  ...
  «24 удалённых листа»). LLM эти разделы не видит (их нет ни в одном чанке), так
  что дублей не возникает.

Состав листов ВНУТРИ общего scope детерминированно НЕ выносится: эти scope_keys
попадают в section split, и LLM сообщает per-sheet added/removed точнее (с
отметками/толщинами). Иначе возникали бы дубли к LLM-выводу (acceptance review
КР2: 4 лишних `section_changed`).
...
имеют приоритет (заменяют LLM-дубль).

**Global singleton types** (`_GLOBAL_SINGLETON_TYPES = {stamp_changed}`): из-за
shared global header штамп виден в каждом чанке, и Opus повторяет его смену в
каждом per-chunk ответе, формулируя по-разному — сигнатурная дедупликация их не
ловит. Такие типы схлопываются по `type` (не более одного на весь комплект),
приоритет — детерминированному. Acceptance review КР2: было 5 `stamp_changed`
(1 det + 4 llm) → стало 1.
