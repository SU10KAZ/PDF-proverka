...
after READY. Backoff resets only after the stream survives a negotiated heartbeat
interval; a Hello/immediate-drop loop therefore remains throttled. Protocol
version rejection also follows close/backoff/reconnect. The client exposes
low-cardinality attempts/success/disconnect/reconnect counters. A real one-minute
idle-stream test proved one connection
