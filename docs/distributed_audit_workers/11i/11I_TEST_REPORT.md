...
| `tests/test_audit_routing_plan_network_e2e.py` | 3 | контракт «центр → задание → воркер» на настоящих объектах |

### Чего тесты НЕ поймали

Отдельная строка, потому что это важнее списка пройденных пунктов.

Состязательное ревью (`11I_ADVERSARIAL_REVIEW.md`) нашло **пять уникальных
критических дефектов**, из которых три делали ключевые части этапа
неработающими. Ни один не проявлялся падением теста: все три выглядели бы в
бою как штатный fail-soft — судья оплачен и выброшен, страж отсутствия молча
не выполнен, нога ансамбля не прошла проверку после оплаты.

Причина одна и та же: это ошибки СОГЛАСОВАНИЯ двух сторон контракта, а не
ошибки логики внутри одной. Тест, написанный автором обеих сторон, повторяет
его же представление о том, как они стыкуются.
...
четыре файла 11I): **1258 пройдено, 3 упало**.
