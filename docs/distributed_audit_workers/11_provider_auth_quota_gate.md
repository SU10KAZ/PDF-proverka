...
**Выполнено на этом этапе: НОЛЬ запросов.** Ни к Claude, ни к Codex. Причина
проста: авторизация не выполнена, а без неё запрос бессмыслен; и отдельно —
разрешение оператора не запрашивалось до завершения кода.
...
**Решение о типе учётной записи — за оператором**, и оно фиксируется в поле
`account_kind` учётной записи; новая запись заводится в состоянии
`review_required`, пока человек не выберет.
...
1. оператору выбрать тип учётной записи и войти (раздел 8);
2. оператору задать `account_group_id` и `account_kind`;
