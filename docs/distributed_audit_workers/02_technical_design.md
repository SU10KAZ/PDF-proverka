...
|---|---|---|---|
| **Исполнение** (`state`) | `JobStateStore` | **15 значений**, полный перечень — §10.2 | что можно делать с заданием |
...
**Полный перечень состояний исполнения (15).** Число проверяемо: ниже перечислены все значения, и каждое присутствует в диаграмме §10.2 и в таблице переходов §10.3.

| # | `state` | Терминальное | Смысл |
|---|---|---|---|
| 1 | `created` | нет | задание заведено оператором, попытка ещё не выпущена |
| 2 | `assigned` | нет | выпущены `attempt_id` и `execution_token`, source-пакет собран |
| 3 | `source_uploading` | нет | воркер тянет source-пакет (`Range`-GET) |
| 4 | `source_ready` | нет | sha256 архива сошёлся у воркера |
| 5 | `accepted_by_worker` | нет | манифест проверен, дерево распаковано, `POST /accept` принят |
| 6 | `running` | нет | конвейер идёт на воркере |
| 7 | `completed_locally` | нет | конвейер дошёл до границы отложения, архив результата на диске воркера |
| 8 | `result_uploading` | нет | идёт чанкованная передача результата |
| 9 | `result_received` | нет | архив собран на центре, sha256 сошёлся |
| 10 | `validating` | нет | идут четыре проверки §12.7 |
| 11 | `completed` | **да** | результат принят и проверен, continuation поставлен |
| 12 | `failed` | **да** | ошибка воркера, провал валидации или ручное «признать потерянной» |
| 13 | `cancel_requested` | нет | оператор запросил отмену, ждём подтверждения воркера |
| 14 | `cancelled` | **да** | воркер подтвердил отмену |
| 15 | `superseded_result_received` | **да** | результат **отозванной** попытки принят на хранение в `superseded_results/`; не публикуется никогда (§9.4 п.6, §11.2 эндпоинт 16, сценарий 10 §18.2) |

Состояние 15 достижимо **только** для записи отозванной попытки — у актуальной попытки его быть не может. Оно вынесено в общий перечень, а не оставлено «неявным», потому что на него ссылаются три места проекта и по нему фильтруется UI-конфликт.
...
stateDiagram-v2
    ...
    state "ОТОЗВАННАЯ ПОПЫТКА (att_N-1)" as SUP {
        [*] --> superseded_result_received: старый воркер вернулся<br/>с готовым результатом →<br/>POST /superseded-result
        superseded_result_received --> [*]
    }

    note right of SUP
        Отдельная ветка: касается ТОЛЬКО
        записи отозванной попытки.
        В superseded_results/, публикации нет.
        Актуальная попытка не затрагивается.
    end note
...
| `failed` (отозванной попытки) | `superseded_result_received` | W | `POST /jobs/{id}/superseded-result` от вернувшегося старого воркера | пакет → `superseded_results/<job_id>/<attempt_id>/`; **актуальная попытка не затрагивается**; UI показывает конфликт |
...
#### Вычисляемый признак `retention_unconfirmed`

У «висящего» результата — того, что лежит на воркере, но не подтверждён центром, — **нет** и не должно быть собственного состояния исполнения: `state` такого задания вполне может быть `completed_locally`, `result_uploading` или даже `failed`, и подменять его новым значением означало бы смешать два независимых факта (ровно та ошибка, от которой защищает §10.1).

Поэтому вводится **вычисляемый признак**, а не состояние:

```
retention_unconfirmed = (на воркере есть материализованный результат)
                        AND (retention_until IS NULL)
```

| Свойство | Значение |
|---|---|
| Где вычисляется | на **обеих** сторонах независимо: воркер — по своему `metadata.json`, центр — по `remote_jobs.validated_at IS NULL` при полученном `job_completed_locally` |
| Хранится ли | **нет** — ни колонки, ни поля в enum; это предикат над уже имеющимися данными |
| Что означает | «Центр не подтвердил приём — автоматическое удаление запрещено» |
| Что запрещает | `RetentionManager` не имеет права удалять такое задание **ни при каких условиях**, включая нехватку диска (§17.4: воркер уходит в `degraded`, но сам ничего не удаляет) |
| Как видно оператору | предупреждение в карточке VPS: «⚠ N результатов без подтверждения приёма, занято X ГБ» + пометка в строке задания |
| Как снимается | приходом `retention_until` (то есть после `validated_at`) либо ручным `delete_package` оператора — и только им |
| Тест | `test_retention_requires_ack` (§24.4 I-21) проверяет, что признак взведён и удаление отбито |

Это ровно та защита, что описана в §19.4 «Что если центр никогда не подтвердил?», но названная и проверяемая, а не оставленная в тексте.
...
| Вопрос | Ответ |
|---|---|
| Что если центр никогда не подтвердил? | взводится вычисляемый признак `retention_unconfirmed` (§10.6): пакет хранится **бессрочно**, авто-удаление запрещено; на экране «Воркеры» — предупреждение «Центр не подтвердил приём — автоматическое удаление запрещено» |
...
| Машина состояний | **три независимые оси**: исполнение (15 состояний, перечислены поимённо в §10.1), связь (4), хранение (4) + вычисляемый признак `retention_unconfirmed`. Молчание меняет только ось связи и **никогда** не переводит `running → failed` |
