...
**Статус:** УДАЛЕНО из интерфейса 20.08.2026 по решению заказчика. Сняты
тумблер «🧪 Новый алгоритм (диагностика)», витрина артефактов, колонка
расхождений напротив листов, подсветка change group и оба endpoint'а
`diagnostic/new-pipeline` (описанные ниже запросы отвечают 404).
Модуль `diagnostic_new_pipeline.py`, `store.get_new_pipeline_diagnostic` и
`store.render_new_pipeline_crop` оставлены нетронутыми — документ описывает
их логику и годится как спецификация при возврате режима.

Введено 19.08.2026, снято 20.08.2026.
...
Кнопка «→» в таблице и клик по карточке в колонке расхождений открывают
вкладку «Связь блоков» и **центрируют оба листа на самой области**, а не просто
прокручивают к листу: на крупном масштабе «где-то на этом листе» означало искать
глазами. **Масштаб при этом не меняется** — какой выставил пользователь, такой и
остаётся.

Подсветка на **обеих** страницах:

* **красная сплошная рамка** — сама change group;
* **синий пунктир** — atomic regions внутри неё.

Центрирование считается по фактическим экранным прямоугольникам рамки и панели
(`_scCenterPaneOnElement`), поэтому работает на любом zoom и на каждой стороне
по своей рамке — у правого листа та же область стоит в других координатах.
Рамка рисуется только для отрисованного слота, поэтому порядок такой: подвести
слот → дождаться появления рамки (до 2 с) → центрировать. Если рамка так и не
появилась, прокрутка честно остаётся на уровне слота.

Область у самого края листа центрируется настолько, насколько позволяет
прокрутка: браузер упирается в границу контента — это не сбой, область всё
равно видна.

Над панелями появляется плашка с кнопкой «Снять подсветку».

## Прокрутка колонки — независимая

Колонка **не** синхронизирована с листами попиксельно: пока она ехала за ними,
её дёргало от любой прокрутки внутри одного листа. Сейчас:

* у колонки **свой ползунок**, карточки растут по содержимому (раньше слот с 19
  группами вжимался в высоту листа);
* прокрутка листов вверх-вниз колонку **не трогает**;
* колонка подъезжает к нужному разделу **только при переходе на другой лист** —
  единственный триггер это смена `scVisibleSlotLeft`;
* текущий лист помечен в колонке фиолетовой рамкой и надписью «● на экране»,
  заголовок раздела липкий.

«Текущий лист» определяется тем же способом, что и виртуализация: слот, чей
центр ближе всего к 40 % высоты левой панели. Пересчёт висит не только на
событии прокрутки, но и на устаканивании вёрстки (`ResizeObserver`, дебаунс
120 мс): картинки листов грузятся асинхронно и растут уже после прокрутки — без
этого «текущим» оставался лист, каким его застали до раскладки, и колонка
показывала соседний.

Колесо над колонкой: обычное крутит саму колонку, **Ctrl+колесо остаётся зумом
листов** — иначе выходит ловушка, когда курсор увели к списку и крутят с
зажатым Ctrl, а масштаб не меняется. Над колонкой привязываться к точке чертежа
не к чему, поэтому зум идёт от центра панели страниц.
