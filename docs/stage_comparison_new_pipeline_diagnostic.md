...
## Колесо над колонкой расхождений

Обычное колесо прокручивает листы (своего скролла у колонки нет — она следует
за ними). **Ctrl+колесо остаётся зумом**: иначе выходит ловушка — пользователь
ведёт курсор к списку расхождений, продолжает крутить с зажатым Ctrl, и вместо
изменения масштаба листы уезжают вниз. Над колонкой привязываться к точке
чертежа не к чему, поэтому зум идёт от центра панели страниц.
