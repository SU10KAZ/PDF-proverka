...
**7**, не 8. В отчёте по Фазе 6 стояло «8 isolated commits» при списке из
...
На сегодня **единственный** production-required коммит — `8fb78e65`
(ёмкость по владению линией в `function_lineage_shadow.py`). Всё остальное
из этой сессии — стенды, артефакты и инфраструктура тестов.
