...
### 12. `ef292aa8` — пересобранное доказательство эквивалентности

* **Класс:** RESEARCH ONLY
* **Вердикт:** после правок Фаз A и C кандидаты снова пересобираются
  байт-в-байт.

### 13. `d653c92c` — тированная приёмочная оценка (Фаза F)

* **Класс:** RESEARCH ONLY
* **Файлы:** `experiments/function_lineage_v2/acceptance.py` (раннер),
  `experiments/function_lineage_v2/acceptance_metrics.py`,
  `tests/test_function_lineage_v2_acceptance{,_metrics}.py`, результаты
* **Тесты:** 12 + 12
* **Вердикт:** 194/194 вызова успешны, безопасность чиста, но оба
  решающих тира провалили порог 0,90 → **NOT_READY**. Допуск не заработал
  ни один класс.

### 14. Backlog покрытия (Фаза G)

* **Класс:** RESEARCH ONLY (документация)
* **Файл:** `docs/function_lineage_coverage_backlog.md`

### 15. Идентичность экземпляра функции (трек v2.7, Фазы 1/3/7/8)

* **Класс:** RESEARCH ONLY
* **Файлы:** `experiments/function_lineage_v2/instance_identity.py`,
  `tests/test_function_instance_identity.py`, артефакт обследования
* **Тесты:** 15
* **Вердикт:** извлечение идентичности работает и ловит противоречия
  документа, но на этом корпусе сигнала не хватает: 0 из 12 оспоренных
  кластеров разрешено, сертифицированный тир 1:1 пуст.

### 16. Сертификат слияния (трек v2.8, Фазы 1–5, 7–9)

* **Класс:** RESEARCH ONLY
* **Файлы:** `experiments/function_lineage_v2/merge_certificate.py`,
  `tests/test_function_merge_certificate.py`, аудит корпуса
* **Тесты:** 17, включая положительный контроль
* **Вердикт:** из 125 MERGED-кандидатов сертифицировано 0,
  противоречиво 56, частично 69. Тир `AUTO_MERGED_CERTIFIED` пуст.
...
### 17. Восстановление привязки к объекту для MERGED

* **Класс:** RESEARCH ONLY
* **Файлы:** `experiments/function_lineage_v2/object_binding.py`,
  `tests/test_function_object_binding.py`,
  `docs/function_lineage_object_binding.md`, артефакт корпуса
* **Тесты:** 15
* **Вердикт:** `E_DATA_LIMITED`. Разрыв восстановления реален (32 листа из
  277), но ни одна восстановленная привязка не атрибутируется функции: из 26
  листов с названием объекта 24 несут несколько функций. Ни один из 69
  частичных сертификатов не становится сертифицированным. Поле `stamp.Object`
  отвергнуто как доказательство: оно не различает ничего и сертифицировало бы
  сразу все 69.

### 18. Детерминированная привязка доказательств к фрагменту функции (трек v2.9)

* **Класс:** RESEARCH ONLY
* **Файлы:** `experiments/function_lineage_v2/evidence_binding.py`,
  `tests/test_function_evidence_binding.py`,
  `docs/function_lineage_evidence_binding.md`, артефакты корпуса
* **Тесты:** 30, включая все шесть отрицательных контролей Фазы 6
* **Вердикт:** `B` — общий детерминированный слой привязки существует и
  безопасен (1034 из 4213 задокументированных значений привязаны ровно к
  одному фрагменту, близость не привязывает никогда, 0 нарушений инвариантов,
  два прогона байт-в-байт), но добирается не до тех фактов. Ни один
  scope-факт (`serviced_object`, `building`, `corpus`, `section`) не является
  фрагмент-локальным ни в одном корпусе, тиры `AUTO_MERGED_CERTIFIED` и
  `AUTO_ONE_TO_ONE_CERTIFIED` остаются пустыми, а наложение фрагмент-локальных
  паспортов уничтожает все 56 задокументированных опровержений — это потеря
  безопасности, поэтому оно и остаётся измерением. Побочные находки: `D` —
  фрагмент не является областью листа (на 134 из 169 многофункциональных
  страниц предложения фрагментов пересекаются, фрагмента оборудования не
  существует); `E` — для scope-полей источник действительно не содержит
  пофункциональной информации: объект напечатан один раз на лист, в штампе.
